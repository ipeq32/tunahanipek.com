...
const emptyContentByLocale = (): ContentByLocale => ({
  tr: '',
  en: '',
});
...
export function AddSiteCopyLineDialog({
  ...
  const topicId = useId();
  const [contentByLocale, setContentByLocale] =
    useState<ContentByLocale>(emptyContentByLocale);
  const [missingLocales, setMissingLocales] = useState<LocaleCode[]>([]);
  ...
  useEffect(() => {
    ...
    setContentByLocale(emptyContentByLocale());
    setMissingLocales([]);
    setTopic('');
    setIsActive(true);
    setAiBusy(null);
  ...
  function updateLocaleContent(code: LocaleCode, value: string) {
    setContentByLocale((prev) => ({ ...prev, [code]: value }));
    if (missingLocales.includes(code) && value.trim()) {
      setMissingLocales((prev) => prev.filter((locale) => locale !== code));
    }
  }
  ...
  async function runGenerateBoth() {
    ...
    try {
      ...
      const next = { ...emptyContentByLocale() };
      ...
      toast.error(
        error instanceof Error ? error.message : tAi('error')
      );
    } finally {
      setAiBusy(null);
    }
  ...
  async function runTranslate(
    ...
    const sourceLine = contentByLocale[sourceLocale].trim();
    if (!sourceLine) {
      return;
    }
    ...
    try {
      ...
      updateLocaleContent(targetLocale, translated);
  ...
  async function runImprove(code: LocaleCode) {
    const line = contentByLocale[code].trim();
    if (!line) {
      return;
    }
    ...
    try {
      ...
      updateLocaleContent(code, data.item);
  ...
  const handleSubmit = () => {
    const trimmed = Object.fromEntries(
      locales.map((code) => [code, contentByLocale[code].trim()])
    ) as ContentByLocale;

    const empty = locales.filter((code) => !trimmed[code]);
    if (empty.length > 0) {
      setMissingLocales(empty);
      return;
    }
        ...
        <div className="max-h-[min(62vh,560px)] space-y-4 overflow-y-auto px-6 py-5">
          ...
          <div className="space-y-4">
            {locales.map((code) => {
              ...
              const hasContent = Boolean(contentByLocale[code].trim());
              const canTranslateFromOther =
                showAiPanel &&
                otherLocale &&
                contentByLocale[otherLocale].trim() &&
                !hasContent;
                ...
                <div key={code} className="space-y-2">
                  <div className="flex flex-wrap items-center justify-between gap-2">
                    <Label htmlFor={contentId}>
                      {t('addLineContentLabel', { locale: code.toUpperCase() })}
                    </Label>
                  ...
                  <Textarea
                    id={contentId}
                    value={contentByLocale[code]}
                    onChange={(event) =>
                      updateLocaleContent(code, event.target.value)
                    }
                    rows={4}
                    ...
                    className={cn(
                      ...
                      missingLocales.includes(code) &&
                        'border-destructive focus-visible:ring-destructive/30'
                    )}
                  />
                  {missingLocales.includes(code) ? (
                    <p className="text-xs text-destructive">
                      {t('addLineContentRequired', {
