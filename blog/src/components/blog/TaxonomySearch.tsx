...
type TaxonomyScope = 'tag' | 'category';
...
/**
 * Tag/category sayfalarında aktif taksonomiyi koruyarak metin araması yapar.
 * Aramayı temizlemek mevcut taksonomi sayfasına `q` olmadan döner.
 */
export default function TaxonomySearch({
  ...
  const navigate = (q?: string) => {
    const queryParams = { page: '1', ...(q ? { q } : {}) };
    // Branch ederek next-intl'in tipli pathname/params eşleşmesini koruruz.
    if (scope === 'tag') {
      router.push({
